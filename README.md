...
对 `main` 的 **push**/**pull_request** 会触发 **Android CI**（见 `.github/workflows/android-build.yml`）：安装 SDK、执行 `./gradlew :app:assembleDebug`、`:app:assembleRelease` 与 `:app:testDebugUnitTest`，并将构件 **`app-debug-apk`**（`app-debug.apk`）与 **`app-release-apk`**（`app-release.apk`）上传到该次运行的 **Artifacts**。默认 CI 上的 release 为 **debug 证书签名**；若要在 CI 中使用正式证书，请在仓库 **Settings → Secrets and variables → Actions** 中配置上述 `ANDROID_KEYSTORE_*`（并将 keystore 以安全方式注入工作流，例如 base64 秘密 + 步骤内解码写文件——需你自行权衡，本仓库未内置该步骤）。
