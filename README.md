...
- 仅 **腾讯云 COS**，无多云。
- 以 **MediaStore 可见**音频为主；**小米系**还会扫描公共目录 **`MIUI/sound_recorder`**、**`Recordings/SoundRecorder`** 等；在授予 **全部文件访问** 后额外扫 **`Android/data`/`Android/media` 下录音相关包**（包名启发式已收紧，避免误扫飞书等）。**m4a 等格式不排除**；会按路径排除 **飞书/Lark**（`RELATIVE_PATH`/`DATA`/文件名）以免与会议录音混淆。扫描 **Audio.Media** 与 **Files（MEDIA_TYPE_AUDIO）**；**IS_PENDING** 接受 **NULL** 与 `0`；不按 MIME 在 SQL 里限制为 `audio/*`；跳过 `video/*`。上传支持 **`content://`** 与 **`file://`**。
