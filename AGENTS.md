...
- **GitHub Actions**：`android-build.yml` 在 push/PR 到 `main` 时上传 **Artifacts**。**`android-publish-latest.yml`** 在 **Android CI** 于 `main` 上 **push** 或 **workflow_dispatch** 且 **成功** 后运行，更新 **`app-latest`** Release（`weight-agent-release.apk`），避免在 **PR** 的同一工作流里出现「发布 job 被跳过」。`android-release.yml` 在推送 **`v*`** 标签时创建版本化 Release 并附上 `app-release.apk`。
- 工作流使用 **Node.js 24** 系 Actions，并设置 `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true`，以减少 Node 20 弃用告警。
