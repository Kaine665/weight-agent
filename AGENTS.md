...
- **GitHub Actions**：`android-build.yml` 在 push/PR 到 `main` 时上传 **Artifacts**（含 debug 与 release APK）。`android-release.yml` 在推送 **`v*`** 标签时创建 **GitHub Release** 并附上 `app-release.apk`。
