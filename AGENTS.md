...
- **GitHub Actions**：`android-build.yml` 在 push/PR 到 `main` 时上传 **Artifacts**；在 **push 到 `main` 成功** 后还会更新 **`app-latest`** 发布（Releases 页「Latest build (main)」+ `weight-agent-release.apk`）。`android-release.yml` 在推送 **`v*`** 标签时创建版本化 **GitHub Release** 并附上 `app-release.apk`。
