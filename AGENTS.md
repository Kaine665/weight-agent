...
- **GitHub Actions**：`android-build.yml` 在 push/PR 到 `main` 时上传 **Artifacts**；在 **push 到 `main`**（且提交不含 `[skip ci]`）时先运行 **`scripts/bump_app_version.py`** 自增 `versionCode` / `versionName` patch，构建成功后回推 `main`；**`android-publish-latest.yml`** 随后更新 **`app-latest`** Release，**标题为 `v{versionName}`** + `weight-agent-release.apk`。`android-release.yml` 在推送 **`v*`** 标签时另建 **GitHub Release** 并附上 `app-release.apk`。
